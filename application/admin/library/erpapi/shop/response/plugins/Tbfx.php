 ...
 *
 * 淘宝分销
 * @todo：根据淘宝上的货品，获取关联的基础物料,并插入到sdb_ome_order_items明细层里；
 * @todo：有普通商品、PKG捆绑商品、gift赠品、lkb福袋
 *
...
class Tbfx extends Abstracts
{
    ...
    public function postCreate($order_id,$tbfx)
    {
        ...
        $objectList = $orderLib->getOrderObjects($filter);
        ...
        //订单item层商品列表
        $filter = array('order_id'=>$order_id);
        $itemList = $orderLib->getOrderItems($filter);
